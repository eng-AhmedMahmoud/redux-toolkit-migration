...
import { trackPageView } from 'Container/App/slice';
...
jest.mock<typeof import('Helper/getUserDataHelper')>('Helper/getUserDataHelper', () => ({
    ...jest.requireActual('Helper/getUserDataHelper'),
...
describe('Tracking helper', () => {
    ...
    describe('createTrackingData', () => {
        it('should track data', () => {
            ...
            const createTrackingGenerator = createTrackingData();

            createTrackingGenerator.next();
            createTrackingGenerator.next();

            expect(createTrackingGenerator.next().value)
                .toEqual(put(addTrackingToAction(trackPageView(), TrackType.PageView, expect.anything())));

            expect(createTrackingGenerator.next().done).toBe(true);
        });
    });
});
